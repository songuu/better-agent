...
Direct Agent→SubAgent expansion 按 target kind 分两条 closed 路径。`internal_agent` 要求双方 raw Agent source 与 root Binding exact full pin，先拒绝同 Workspace/kind/resource/version 的直接自循环，再生成 `root → parent binding → subagent_target → dependency-owned binding` 并保留目标 Agent 的完整即时 Binding namespace；它不伪造目标 registry 的 `nested_closure_hash`，传递循环和目标 closure seal 仍由 pinned graph/publisher 证明。`external_a2a` 只接受 `A2A_AGENT_RELEASE` leaf source，并通过批量 leaf verifier 核对 exact pin、manual/schema/operation、credential/classification/transport 约束，然后生成 terminal `root → binding → subagent_target`，不得虚构内部 Binding。两条路径都先登记完整 parent root namespace；显式 parent/target Binding disabled 与最终 delegation-policy unavailable 继续分离。
