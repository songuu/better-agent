...
First recompute the pinned dependency graph and prove the direct parent-to-child edge. Verify the supplied child closure against the dependency node's version identity and `nested_closure_hash`, including its canonical set order and complete closure hash. Resolve every child operation through the exact child-root canonical Binding path and confirm its kind and full target pin. Only then project that operation set onto each already-derived parent-prefixed `subagent_target` Binding path.
