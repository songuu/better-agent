...
> Status: T1 implemented and locally reviewed; T2–T6, final PostgreSQL validation and deployment remain pending.
> Updated: 2026-09-02
...
| Effective capability policy | Structural schema; egress and budget embed JSON | Closed versioned vocabularies plus narrowing-only meet | Closure compiler and admission |
...
- [ ] **T2 — Implement versioned policy meet.** Define closed egress and budget vocabularies; validate credential provider/audience/scope, principal mode intersections, exact operation allow-sets, read clearance, output taint, side-effect maximum/approval and each budget dimension. Files: the closure contract, domain policy schemas and tests, release-core policy module and tests. Depends on T1. Risk: L4.
