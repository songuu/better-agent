...
Current T3.2 increment (still partial): bounded single-resource JSON Schema 2020-12 preparation/verification and real instance validation now have explicit engine/version/profile hashes, isolated worker deadlines/admission and strict non-mutating validation. Operation input/optional output and Strategy five-schema/config-instance wrappers bind complete source validation evidence. **89 new release regressions** pass; observed release-core total is **694/694**, domain baseline remains **152/152**. See `.handoff/g1-a1-json-schema-review.md` for final gate/review state. The prior Instruction Skill (63 release + nine domain), Strategy (49) and Skill Pack (40) source increments remain valid local evidence; earlier pending-source statements are historical. Final closure compilation, all-kind Schema wiring, canonical paths/routes and effective policies, registry/admission integration and application delivery remain open. Schema validation results and source signatures are not host-attested reviewer evidence. No new PostgreSQL, upload, host-attested or production acceptance is claimed.
