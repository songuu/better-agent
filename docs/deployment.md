...
The production foundation follows the Agent project's immutable-release pattern. A successful CI run on `main` triggers `.github/workflows/deploy-foundation.yml`, uploads the accepted commit, provisions the isolated database, and atomically switches `/opt/better-agent/current`.
...
G0-08 does not contain a public HTTP handler. This deployment therefore exposes no fabricated web endpoint; it deploys the accepted database and migration foundation for the later API runtime.
