...
Set `BETTER_AGENT_MODEL_BASE_URL` when an OpenAI-compatible HTTPS endpoint other than `https://api.openai.com/v1` is required. Set `BETTER_AGENT_MODEL_NAME` to the exact provider-native model accepted by that endpoint (for example, `deepseek-v4-flash`); it is applied to every runtime request while the Studio keeps its validated product-facing model aliases. Use `https://api.deepseek.com` as the DeepSeek OpenAI-compatible base URL.
...
The public deployment job finally verifies `https://songuu.top/better-agent/api/healthz` and the application HTML from the GitHub runner. After installing model credentials, it signs in with the host-private product administrator credential and performs one non-persistent role-assistance request; a non-empty valid model response is required, so a merely present but rejected API key cannot produce a green deployment. The credential, session cookie and model output are not logged. The gateway directory must not advertise the route until those checks pass.
