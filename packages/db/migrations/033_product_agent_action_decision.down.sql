DO $guard$
BEGIN
  IF EXISTS (SELECT 1 FROM public.agent_drafts WHERE strategy_profile ->> 'schema_version' = 'product-agent-strategy/4')
    OR EXISTS (SELECT 1 FROM public.agent_product_releases WHERE strategy_profile ->> 'schema_version' = 'product-agent-strategy/4')
    OR EXISTS (SELECT 1 FROM public.agent_product_runs run,
      LATERAL jsonb_array_elements(run.iteration_trace) item WHERE item ? 'action')
  THEN
    RAISE EXCEPTION 'cannot remove product Agent action decision evidence while v4 policy or action traces exist';
  END IF;
END;
$guard$;

REVOKE EXECUTE ON FUNCTION app.read_agent_product_run_capabilities(uuid,uuid,uuid) FROM ba_runtime;
REVOKE EXECUTE ON FUNCTION app.record_agent_product_run_decision(uuid,uuid,uuid,bigint,text,text,text,text,text,text,text,bigint,bigint) FROM ba_runtime;
DROP FUNCTION app.read_agent_product_run_capabilities(uuid,uuid,uuid);
DROP FUNCTION app.record_agent_product_run_decision(uuid,uuid,uuid,bigint,text,text,text,text,text,text,text,bigint,bigint);
...
AS $function$
  SELECT jsonb_typeof(p_profile) = 'object'
    ...
    AND NOT EXISTS (SELECT 1 FROM jsonb_array_elements(p_profile -> 'routes') route
...
BEGIN
  SELECT run.* INTO v_run FROM public.agent_product_runs run
  ...
  WHERE run.workspace_id=p_workspace_id AND run.id=p_run_id AND run.status='pending'
    AND run.effective_parameters IS NOT NULL AND conversation.actor_id=p_actor_id FOR UPDATE OF run;
  IF NOT FOUND THEN
  ...
  FROM public.agent_product_conversations conversation JOIN public.agent_product_releases release
    ON release.workspace_id=conversation.workspace_id AND release.agent_id=conversation.agent_id
   AND release.version=conversation.release_version
  ...
  IF v_strategy ->> 'schema_version' = 'product-agent-strategy/3' THEN
    ...
    IF btrim(p_output_text) IS DISTINCT FROM v_last ->> 'output_text'
      ...
      OR p_input_tokens IS DISTINCT FROM v_input_total OR p_output_tokens IS DISTINCT FROM v_output_total THEN
      RAISE EXCEPTION 'product Run terminal, iteration or aggregate budget conflict' USING ERRCODE='40001';
    END IF;
  ELSE
  ...
  IF v_input_total+v_run.router_input_tokens+v_run.parameter_input_tokens
    ...
    OR v_output_total+v_run.router_output_tokens+v_run.parameter_output_tokens
       > (v_strategy ->> 'max_output_tokens')::bigint THEN
    RAISE EXCEPTION 'product Run terminal, iteration or aggregate budget conflict' USING ERRCODE='40001';
  END IF;
  UPDATE public.agent_product_runs run SET status='completed',output_text=btrim(p_output_text),
    provider_request_id=p_provider_request_id,input_tokens=v_input_total,output_tokens=v_output_total,
    completed_at=clock_timestamp() WHERE run.workspace_id=p_workspace_id AND run.id=p_run_id
