DO $guard$
BEGIN
 IF EXISTS (SELECT 1 FROM public.product_mcp_server_resources) THEN
  RAISE EXCEPTION 'Cannot roll back product MCP server migration while resources exist';
 END IF;
END;$guard$;
...
DROP FUNCTION app.read_agent_product_run_mcp_server(uuid,uuid,uuid);
DROP FUNCTION app.update_agent_draft_with_strategy_capabilities_v8(uuid,uuid,bigint,text,text,text,text,uuid,uuid,text,jsonb,jsonb,uuid,uuid,uuid,bigint,uuid,bigint);
DROP FUNCTION app.create_agent_draft_with_strategy_capabilities_v8(uuid,uuid,text,text,text,text,uuid,uuid,text,jsonb,jsonb,uuid,uuid,uuid,bigint,uuid,bigint);
DROP FUNCTION app.update_product_mcp_server(uuid,uuid,bigint,uuid,text,text,text,text);
DROP FUNCTION app.create_product_mcp_server(uuid,uuid,text,text,text,text);
DROP FUNCTION app.list_product_mcp_servers(uuid);
DROP TRIGGER agent_product_release_mcp_server_snapshot ON public.agent_product_releases;
DROP FUNCTION app.snapshot_agent_product_release_mcp_server();
...
DROP TABLE public.agent_product_release_mcp_server_bindings;
DROP TABLE public.agent_product_mcp_server_bindings;
DROP TABLE public.product_mcp_server_releases;
DROP TABLE public.product_mcp_server_resources;
