...
DO $guard$
BEGIN
  IF EXISTS (SELECT 1 FROM public.product_custom_api_resources) THEN
    RAISE EXCEPTION 'cannot remove product custom APIs while resources exist';
  END IF;
END;
$guard$;
...
DROP FUNCTION app.assert_product_flow_resources_pinned(uuid, jsonb);
DROP FUNCTION app.assert_product_flow_apis_pinned(uuid, jsonb);
DROP FUNCTION app.update_product_custom_api(uuid, uuid, bigint, uuid, text, text, text, text, text);
DROP FUNCTION app.create_product_custom_api(uuid, uuid, text, text, text, text, text);
DROP FUNCTION app.list_product_custom_apis(uuid);
DROP TABLE public.product_custom_api_releases;
DROP TABLE public.product_custom_api_resources;
