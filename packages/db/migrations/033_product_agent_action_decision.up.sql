-- Strategy v4 lets the model choose a bound read-only capability or terminate.
-- Every interpreted decision and tool result is sealed before the next model call.
...
AS $function$
  SELECT jsonb_typeof(p_profile) = 'object'
    AND CASE
      WHEN p_profile ->> 'schema_version' = 'product-agent-strategy/1' THEN
      ...
      WHEN p_profile ->> 'schema_version' IN ('product-agent-strategy/2','product-agent-strategy/3','product-agent-strategy/4') THEN
        (SELECT array_agg(key ORDER BY key) FROM jsonb_object_keys(p_profile) keys(key))
          = ARRAY['forced_capability','max_input_tokens','max_iterations','max_output_tokens',
    ...
    AND CASE p_profile ->> 'schema_version'
      ...
      WHEN 'product-agent-strategy/4' THEN (p_profile ->> 'max_iterations') ~ '^[1-4]$'
...
CREATE OR REPLACE FUNCTION app.is_valid_product_agent_iteration_trace(p_trace jsonb)
RETURNS boolean LANGUAGE sql IMMUTABLE SET search_path = pg_catalog, pg_temp
AS $function$
  SELECT jsonb_typeof(p_trace) = 'array'
    AND jsonb_array_length(p_trace) BETWEEN 0 AND 4
    AND NOT EXISTS (
      SELECT 1 FROM jsonb_array_elements(p_trace) WITH ORDINALITY item(value, ordinal)
      WHERE jsonb_typeof(item.value) <> 'object'
        ...
        OR NOT (
          (
            (SELECT array_agg(key ORDER BY key) FROM jsonb_object_keys(item.value) keys(key))
              = ARRAY['input_tokens','iteration','model','output_text','output_tokens','provider_request_id']::text[]
            AND jsonb_typeof(item.value -> 'output_text') = 'string'
            AND length(btrim(item.value ->> 'output_text')) BETWEEN 1 AND 50000
          ) OR (
            (SELECT array_agg(key ORDER BY key) FROM jsonb_object_keys(item.value) keys(key))
              = ARRAY['action','capability','input_tokens','iteration','model','output_text','output_tokens',
                      'provider_request_id','tool_input','tool_output']::text[]
            AND item.value ->> 'action' IN ('tool','final')
            AND (
              (item.value ->> 'action' = 'final'
                AND jsonb_typeof(item.value -> 'output_text') = 'string'
                AND length(btrim(item.value ->> 'output_text')) BETWEEN 1 AND 50000
                AND jsonb_typeof(item.value -> 'capability') = 'null'
                AND jsonb_typeof(item.value -> 'tool_input') = 'null'
                AND jsonb_typeof(item.value -> 'tool_output') = 'null')
              OR
              (item.value ->> 'action' = 'tool'
                AND item.value ->> 'capability' IN ('knowledge','database')
                AND jsonb_typeof(item.value -> 'tool_input') = 'string'
                AND length(btrim(item.value ->> 'tool_input')) BETWEEN 1 AND 500
                AND jsonb_typeof(item.value -> 'tool_output') = 'string'
                AND length(item.value ->> 'tool_output') BETWEEN 1 AND 50000
                AND jsonb_typeof(item.value -> 'output_text') = 'null')
            )
          )
        )
    );
...
CREATE FUNCTION app.read_agent_product_run_capabilities(
  p_workspace_id uuid,p_run_id uuid,p_actor_id uuid
) RETURNS TABLE(knowledge boolean,database boolean)
LANGUAGE sql SECURITY DEFINER STABLE SET search_path = pg_catalog, public, pg_temp
AS $function$
  SELECT
    EXISTS (SELECT 1 FROM public.agent_product_release_knowledge_bindings binding
      WHERE binding.workspace_id = conversation.workspace_id
        AND binding.agent_id = conversation.agent_id
        AND binding.release_version = conversation.release_version),
    EXISTS (SELECT 1 FROM public.agent_product_release_database_bindings binding
      WHERE binding.workspace_id = conversation.workspace_id
        AND binding.agent_id = conversation.agent_id
        AND binding.release_version = conversation.release_version)
  ...
  WHERE run.workspace_id = p_workspace_id AND run.id = p_run_id
    AND run.status = 'pending' AND run.effective_parameters IS NOT NULL
    AND conversation.actor_id = p_actor_id
    AND release.strategy_profile ->> 'schema_version' = 'product-agent-strategy/4';
$function$;

CREATE FUNCTION app.record_agent_product_run_decision(
  p_workspace_id uuid,p_run_id uuid,p_actor_id uuid,p_iteration bigint,p_model text,
  p_action text,p_capability text,p_tool_input text,p_tool_output text,p_output_text text,
...
BEGIN
  ...
  WHERE run.workspace_id = p_workspace_id AND run.id = p_run_id AND run.status = 'pending'
    AND run.effective_parameters IS NOT NULL AND conversation.actor_id = p_actor_id
    AND release.strategy_profile ->> 'schema_version' = 'product-agent-strategy/4'
  FOR UPDATE OF run;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'model tool decision, binding, order or aggregate budget conflict' USING ERRCODE = '40001';
  END IF;
  SELECT release.strategy_profile,conversation.agent_id,conversation.release_version
    INTO v_strategy,v_agent_id,v_release_version
  FROM public.agent_product_conversations conversation
  JOIN public.agent_product_releases release
    ON release.workspace_id = conversation.workspace_id
   AND release.agent_id = conversation.agent_id AND release.version = conversation.release_version
  WHERE conversation.workspace_id = p_workspace_id AND conversation.id = v_run.conversation_id;
  IF p_action IS NULL OR NOT (p_action IN ('tool','final'))
    OR p_iteration IS DISTINCT FROM jsonb_array_length(v_run.iteration_trace) + 1
    ...
    OR EXISTS (SELECT 1 FROM jsonb_array_elements(v_run.iteration_trace) item WHERE item ->> 'action' = 'final')
    OR (p_action = 'final' AND (
      p_output_text IS NULL OR length(btrim(p_output_text)) NOT BETWEEN 1 AND 50000
      OR p_capability IS NOT NULL OR p_tool_input IS NOT NULL OR p_tool_output IS NOT NULL))
    OR (p_action = 'tool' AND (
      p_capability IS NULL OR p_capability NOT IN ('knowledge','database') OR p_output_text IS NOT NULL
      OR p_tool_input IS NULL OR length(btrim(p_tool_input)) NOT BETWEEN 1 AND 500
      OR p_tool_output IS NULL OR length(p_tool_output) NOT BETWEEN 1 AND 50000))
  THEN
    RAISE EXCEPTION 'model tool decision, binding, order or aggregate budget conflict' USING ERRCODE = '40001';
  END IF;
  IF p_action = 'tool' AND p_capability = 'knowledge' AND NOT EXISTS (
    SELECT 1 FROM public.agent_product_release_knowledge_bindings binding
    WHERE binding.workspace_id = p_workspace_id AND binding.agent_id = v_agent_id
      AND binding.release_version = v_release_version
  ) THEN
    RAISE EXCEPTION 'model tool decision, binding, order or aggregate budget conflict' USING ERRCODE = '40001';
  END IF;
  IF p_action = 'tool' AND p_capability = 'database' AND NOT EXISTS (
    SELECT 1 FROM public.agent_product_release_database_bindings binding
    WHERE binding.workspace_id = p_workspace_id AND binding.agent_id = v_agent_id
      AND binding.release_version = v_release_version
  ) THEN
    RAISE EXCEPTION 'model tool decision, binding, order or aggregate budget conflict' USING ERRCODE = '40001';
  END IF;
  SELECT count(*) INTO v_tool_count FROM jsonb_array_elements(v_run.iteration_trace) item
    WHERE item ->> 'action' = 'tool';
  IF v_tool_count + (CASE WHEN p_action = 'tool' THEN 1 ELSE 0 END)
       > (v_strategy ->> 'max_tool_calls')::bigint
    OR (p_action = 'final' AND v_strategy ->> 'forced_capability' <> 'none'
      AND NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_run.iteration_trace) item
        WHERE item ->> 'action' = 'tool'
          AND item ->> 'capability' = v_strategy ->> 'forced_capability'))
  THEN
    RAISE EXCEPTION 'model tool decision, binding, order or aggregate budget conflict' USING ERRCODE = '40001';
  END IF;
  SELECT COALESCE(sum((item ->> 'input_tokens')::bigint),0),
         COALESCE(sum((item ->> 'output_tokens')::bigint),0)
    INTO v_input_total,v_output_total FROM jsonb_array_elements(v_run.iteration_trace) item;
  ...
  SET iteration_count = p_iteration,
      iteration_trace = run.iteration_trace || jsonb_build_array(jsonb_build_object(
        'action',p_action,'capability',p_capability,'input_tokens',p_input_tokens,
        'iteration',p_iteration,'model',p_model,'output_text',
        CASE WHEN p_action = 'final' THEN btrim(p_output_text) ELSE NULL END,
        'output_tokens',p_output_tokens,'provider_request_id',p_provider_request_id,
        'tool_input',CASE WHEN p_action = 'tool' THEN btrim(p_tool_input) ELSE NULL END,
        'tool_output',CASE WHEN p_action = 'tool' THEN p_tool_output ELSE NULL END
      ))
  WHERE run.workspace_id = p_workspace_id AND run.id = p_run_id;
END;
$function$;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'product Run terminal, iteration or aggregate budget conflict' USING ERRCODE = '40001';
  END IF;
  IF v_strategy ->> 'schema_version' = 'product-agent-strategy/4' THEN
    ...
    SELECT COALESCE(sum((item ->> 'input_tokens')::bigint),0),
           COALESCE(sum((item ->> 'output_tokens')::bigint),0),
           count(*) FILTER (WHERE item ->> 'action' = 'tool')
      INTO v_input_total,v_output_total,v_tool_count
    FROM jsonb_array_elements(v_run.iteration_trace) item;
    IF v_last ->> 'action' <> 'final'
      OR btrim(p_output_text) IS DISTINCT FROM v_last ->> 'output_text'
      ...
      OR v_tool_count > (v_strategy ->> 'max_tool_calls')::bigint
      OR (v_strategy ->> 'forced_capability' <> 'none' AND NOT EXISTS (
        SELECT 1 FROM jsonb_array_elements(v_run.iteration_trace) item
        WHERE item ->> 'action' = 'tool'
          AND item ->> 'capability' = v_strategy ->> 'forced_capability'))
    THEN
      RAISE EXCEPTION 'product Run terminal, iteration or aggregate budget conflict' USING ERRCODE = '40001';
    END IF;
  ELSIF v_strategy ->> 'schema_version' = 'product-agent-strategy/3' THEN
    IF v_run.iteration_count NOT BETWEEN 1 AND (v_strategy ->> 'max_iterations')::bigint THEN
      RAISE EXCEPTION 'product Run terminal, iteration or aggregate budget conflict' USING ERRCODE = '40001';
    END IF;
    v_last := v_run.iteration_trace -> (v_run.iteration_count::int - 1);
    ...
    IF btrim(p_output_text) IS DISTINCT FROM v_last ->> 'output_text'
      OR p_provider_request_id IS DISTINCT FROM v_last ->> 'provider_request_id'
      OR p_input_tokens IS DISTINCT FROM v_input_total OR p_output_tokens IS DISTINCT FROM v_output_total
    THEN
      RAISE EXCEPTION 'product Run terminal, iteration or aggregate budget conflict' USING ERRCODE = '40001';
    END IF;
  ELSE
    IF v_run.iteration_count <> 0 THEN
      RAISE EXCEPTION 'product Run terminal, iteration or aggregate budget conflict' USING ERRCODE = '40001';
    END IF;
    v_input_total := p_input_tokens; v_output_total := p_output_tokens;
  END IF;
  IF v_input_total + v_run.router_input_tokens + v_run.parameter_input_tokens
       > (v_strategy ->> 'max_input_tokens')::bigint
    OR v_output_total + v_run.router_output_tokens + v_run.parameter_output_tokens
  ...
  WHERE run.workspace_id=p_workspace_id AND run.id=p_run_id RETURNING run.* INTO v_run;
  RETURN v_run;
END;
$function$;
...
ALTER FUNCTION app.record_agent_product_run_decision(uuid,uuid,uuid,bigint,text,text,text,text,text,text,text,bigint,bigint) OWNER TO ba_authorization_owner;
...
REVOKE ALL ON FUNCTION app.read_agent_product_run_capabilities(uuid,uuid,uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION app.record_agent_product_run_decision(uuid,uuid,uuid,bigint,text,text,text,text,text,text,text,bigint,bigint) FROM PUBLIC;
REVOKE ALL ON FUNCTION app.complete_agent_product_run(uuid,uuid,uuid,text,text,bigint,bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION app.read_agent_product_run_capabilities(uuid,uuid,uuid) TO ba_runtime;
GRANT EXECUTE ON FUNCTION app.record_agent_product_run_decision(uuid,uuid,uuid,bigint,text,text,text,text,text,text,text,bigint,bigint) TO ba_runtime;
