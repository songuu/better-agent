DO $guard$
BEGIN
  IF EXISTS (SELECT 1 FROM public.agent_drafts WHERE strategy_profile ->> 'schema_version' = 'product-agent-strategy/3')
    OR EXISTS (SELECT 1 FROM public.agent_product_releases WHERE strategy_profile ->> 'schema_version' = 'product-agent-strategy/3')
    OR EXISTS (SELECT 1 FROM public.agent_product_runs WHERE iteration_count > 0)
  THEN
    RAISE EXCEPTION 'cannot remove product Agent iteration evidence while v3 policy or traces are retained'
      USING ERRCODE = '55000';
  END IF;
END;
$guard$;
...
REVOKE EXECUTE ON FUNCTION app.record_agent_product_run_iteration(uuid,uuid,uuid,bigint,text,text,text,bigint,bigint) FROM ba_runtime;
DROP FUNCTION app.record_agent_product_run_iteration(uuid,uuid,uuid,bigint,text,text,text,bigint,bigint);
...
BEGIN
  ...
  SET status='completed',output_text=p_output_text,provider_request_id=p_provider_request_id,
      input_tokens=p_input_tokens,output_tokens=p_output_tokens,completed_at=clock_timestamp()
  ...
  WHERE run.workspace_id=p_workspace_id AND run.id=p_run_id AND run.status='pending'
    ...
    AND release.workspace_id=conversation.workspace_id AND release.agent_id=conversation.agent_id
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'product Run terminal, parameter or aggregate budget conflict' USING ERRCODE='40001';
  END IF;
  RETURN v_row;
END;
$function$;
...
ALTER TABLE public.agent_product_runs
  DROP CONSTRAINT agent_product_runs_iteration_trace_check,
  DROP COLUMN iteration_trace,
  DROP COLUMN iteration_count;
DROP FUNCTION app.is_valid_product_agent_iteration_trace(jsonb);
