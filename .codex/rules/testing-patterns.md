...
- Nested policy evidence 不能只用空 requirements 测字段存在；至少重封一次含非零 minimum/read classification 的 child closure、更新 graph nested hash，并断言 parent dependency node 逐字保留该 policy 且 deep-frozen。未定义 composite minima 公式前，不应写 parent entry 的“通过”测试来固化猜测。
